...
// ---------------------------------------------------------------------------
// Local badge components — render status and priority inline in table/cards.
// ---------------------------------------------------------------------------
...
export default function CasesListPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(getUserFriendlyError(err, 'Failed to create case.'));
    } finally {
      setCreating(false);
    }
  };
      ...
      <div className="bg-medical-surface border border-medical-border rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-medical-text/70 text-sm">No cases found</p>
            <p className="text-medical-text/60 text-xs mt-1">Create your first case to get started</p>
          </div>
        ) : (
          <>
                ...
                <thead>
                  <tr className="border-b border-medical-border">
                    {['Case Code', 'Patient', 'Status', 'Priority', 'Scans', 'AI Result', 'Doctor', 'Date'].map(h => (
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-medical-text/40 font-medium text-xs uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
            ...
            >
              <div className="flex items-start justify-between mb-6">
                ...
                <button
                  ...
                  className="text-medical-text/40 hover:text-medical-text transition-colors mt-1"
                  aria-label="Close"
                >
